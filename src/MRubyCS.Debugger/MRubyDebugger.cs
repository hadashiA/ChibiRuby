...
public sealed class MRubyDebugger(MRubyState mrb, MRubyCompiler compiler) : IMRubyDebuggerHook, IDisposable
{
    ...
    void HandleEval(RBinding binding, EvalCommand cmd)
    {
        ...
        var bindingGlobal = mrb.Intern("$__mrubycs_dbg_binding"u8);
    ...
    string BuildBindingScopedSource(RBinding binding, string userSource)
    {
        ...
        // to using `$__mrubycs_dbg_binding` directly).
        ...
        if (!userHasBinding)
        {
            sb.Append("binding=$__mrubycs_dbg_binding;");
        }
        ...
        foreach (var name in names)
        {
            ...
            sb.Append(ident);
            sb.Append("=$__mrubycs_dbg_binding.local_variable_get(:");
            sb.Append(ident);
            sb.Append(");");
        }
