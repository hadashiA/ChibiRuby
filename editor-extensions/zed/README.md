...
- the adapter name `mruby-cs` exists,
...
5. In Zed, open the Debug panel (`cmd-shift-d`) → pick **Attach to mruby/cs** → run.
   Breakpoints in `.rb` files become active once the session is live.

## debug.json fields

- `adapter` (**required**): must be `"mruby-cs"` for this extension to handle it.
