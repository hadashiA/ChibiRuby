...
public class JsonTest
{
    ...
    {
        // Single-quoted Ruby string: backslashes reach the JSON parser intact.
        Assert.That(EvalString("""JSON.parse('"a\nb\tc\"d\\\\e"')"""),
            Is.EqualTo("a\nb\tc\"d\\e"));
    }

    [Test]
    public void Parse_UnicodeEscape_ToUtf8()
    {
        // \u3042 = あ (3-byte UTF-8). C# raw string + Ruby single quotes keep
        // the backslash literal all the way to the JSON parser.
        Assert.That(EvalString("""JSON.parse('"\u3042"')"""), Is.EqualTo("あ"));
        // Surrogate pair \uD83D\uDE00 = 😀 (4-byte UTF-8, astral plane)
        Assert.That(EvalString("""JSON.parse('"\uD83D\uDE00"')"""), Is.EqualTo("😀"));
    }

    [Test]
    public void Parse_EscapedPropertyName()
    {
        var script = """JSON.parse('{"a\nb":1}')""";
        var hash = Eval(script).As<RHash>();
        Assert.That(hash.TryGetValue(new MRubyValue(mrb.NewString("a\nb")), out var v), Is.True);
        Assert.That(v.IntegerValue, Is.EqualTo(1));
    }

    [Test]
    public void RoundTrip_RawUtf8_PassesThrough()
    {
        // Non-ASCII UTF-8 is emitted raw (not \u-escaped) and parses back.
        Assert.That(EvalString("JSON.generate(\"日本語\")"), Is.EqualTo("\"日本語\""));
        Assert.That(EvalString("JSON.parse(JSON.generate(\"日本語😀\"))"), Is.EqualTo("日本語😀"));
    }

    [Test]
    public void Generate_ControlChars_Escaped()
    {
        Assert.That(EvalString("JSON.generate(\"a\nb\")"), Is.EqualTo("\"a\\nb\""));
        Assert.That(EvalString("JSON.generate(\"q\\\"w\")"), Is.EqualTo("\"q\\\"w\""));
    }

    [Test]
    public void Parse_NaN_WithAllowNan()
    {
        // NaN != NaN — the canonical NaN check without Float#nan?.
        var script = """
                     v = JSON.parse("NaN", allow_nan: true)
                     v != v
                     """;
        Assert.That(Eval(script).VType, Is.EqualTo(MRubyVType.True));
        var inf = Eval("""JSON.parse("-Infinity", allow_nan: true)""");
        Assert.That(double.IsNegativeInfinity(inf.FloatValue), Is.True);
    }

    [Test]
    public void Parse_NaN_WithoutAllowNan_Raises()
    {
        var script = """
                     begin
                       JSON.parse("NaN")
