...
public class MRubyDebuggerTest
{
    ...
    public void Eval_DoesNotLeakTemporaryGlobal()
    {
        ...
        // The wrapper relies on $__mrubycs_dbg_binding internally; it must be cleared
        // after the eval session so callers can't accidentally observe it.
        Assert.That(mrb.GlobalVariableDefined(mrb.Intern("$__mrubycs_dbg_binding"u8)), Is.False);
    }
